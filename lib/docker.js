  ...
  return config.get()
    .then((configuration) => {
      ...
      _.each(configuration.dockerContainers, dockerContainer => {
        const serviceVarName = serviceNameToVarName(dockerContainer.name)
        addEnvironmentContainerVars(envVars, serviceVarName, {
          command: '',
          command_params: '',
          coverage_enabled: '',
          wait_for: '',
...
const composeEnvVars = function (test, suiteName) {
  return config.get()
    .then((configuration) => {
      ...
      addEnvironmentContainerVars(vars, testVarName, {
        command: testCommand,
        command_params: testCommandParams,
        coverage_enabled: testCoverageIsEnabled,
        wait_for: getWaitFor(test.test),
        exit_after: getExitAfter(test.test)
      })
      ...
      _.each(test.services, (service) => {
        ...
        addEnvironmentContainerVars(vars, varName, {
          ...
          coverage_enabled: coverageIsEnabled(test, service.name),
...
const run = function (suite, suiteName) {
  ...
  return composeEnvVars(suite, suiteName)
    .then((envVars) => {
      const runDownVolumes = suite.before && suite.before.docker && suite.before.docker['down-volumes'] ? downVolumes() : Promise.resolve()
      const exitCodeFrom = getExitCodeFrom(suite)
