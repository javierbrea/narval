...
const composeEnvVars = function (test, suiteName) {
  ...
  const testCommandAndParams = istanbulMocha.getCommandAndParams(test.test.commands && test.test.commands.docker)
  ...
  let testCommandParams = testCommandAndParams ? testCommandAndParams.params : istanbulMocha.mocha.params(test)
  ...
  _.each(test.services, (service) => {
    const commandAndParams = istanbulMocha.getCommandAndParams(service.commands.docker)
