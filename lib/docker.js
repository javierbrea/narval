...
const runServiceContainer = function (serviceDockerConfig, suiteData, envVars) {
  tracer.debug(`Starting Docker service "${suiteData.service}"`)
  return runComposeSync(`start ${serviceDockerConfig.container}`, {
    env: envVars
  })
    .then(() => {
      return new Promise((resolve, reject) => {
        ...
        proc.logs = new processLogs.Handler(proc, {
          type: suiteData.type,
        ...
        resolve({
          ...
          process: proc,
          abortOnExit: serviceDockerConfig['abort-on-exit']
        })
      })
    })
}
...
const getContainerAndRun = function (suite, suiteTypeName, serviceName, envVars) {
  ...
  if (serviceName === 'test') {
    return Promise.reject(`There is no Docker configuration for test in suite "${suite.name}" of type "${suiteTypeName}"`)
  }
