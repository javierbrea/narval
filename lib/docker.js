...
const getDockerComposeEnvVars = function (test, suiteName) {
  ...
  vars.push(testVarName + '_wait_for="' +  test.test['wait-for'] + '"')
  ...
  _.each(test.services, (service) => {
    ...
    vars.push(varName + '_wait_for="' + service['wait-for'] + '"')
  })
...
const run = function (test, suiteName, options) {
  ...
  const dockerOptions = options.build ? '--build' : ''
  try {
    ...
    childProcess.execSync(dockerComposeCommand + ' up ' + dockerOptions, {
      cwd: paths.cwd.resolve(DOCKER_PATH),
      stdio: [0, 1, 2],
      shell: true,
      windowsHide: true
