...
const composeEnvVars = function (test, suiteName) {
  ...
  const testCommandAndParams = istanbulMocha.getCommandAndParams(test.test.docker && test.test.docker.command)

  let testVarName = serviceNameToVarName(test.test.docker.container)
