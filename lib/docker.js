...
const getCoverageOptions = function (test, suiteName) {
  ...
  options.push('|cov-option|dir=' + suiteName + '/' + test.name)
  return options.join('')
}
...
const getDockerComposeEnvVars = function (test, suiteName) {
  ...
  vars.push('coverage_options="' + getCoverageOptions(test, suiteName) + '"')
  vars.push(testVarName + '_coverage_enabled="' + coverageIsEnabled(test, test.test['docker-service'], true) + '"')
  ...
  _.each(test.services, (service) => {
    ...
    vars.push(varName + '_coverage_enabled="' + coverageIsEnabled(test, service['docker-service']) + '"')
  })
...
const downVolumes = function (envVars) {
  if(hasRun) {
    ...
    childProcess.execSync(envVars + 'docker-compose -f docker-compose.json down --volumes', {
