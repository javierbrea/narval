...
const getCoverageOptions = function (test, suiteName) {
  return '--include-all-sources --print=detail --dir=.coverage/' + suiteName + '/' + test.name
}
...
const composeEnvVars = function (test, suiteName) {
  ...
  let testCommandParams = testCommandAndParams ? testCommandAndParams.params : '-- --recursive ' + test.test.specs
