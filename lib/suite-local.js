...
const Runner = function (config) {
  ...
  const runSingleService = function () {
    // TODO, change
    return runService(config.suite(), name, type, config.options(), true)
      .then(resolveOnClose)
  }

  const _runTest = function () {
    // TODO, change
    return runTest(config.suite(), name, type)
  }

  const run = function () {
    const singleServiceToRun = config.runSingleService()
    ...
    return runBefore(config.suite(), name, type)
      // .then(runServicesAndTest)
      .then(() => {
        return runServicesAndTest(config.suite(), name, type)
      })
  }
