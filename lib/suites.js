...
const Logger = function (suiteConfig) {
  ...
  const loggers = {
    ...
    finishOk: new Log(`Execution of "${suiteTypeName}" suite "${name}" finished OK`, 'info'),
...
const Suite = function (suiteData, suiteTypeName, options) {
  ...
  const runDocker = function () {
    return docker.createFiles()
      .then(() => {
        return docker.run(suiteData, suiteTypeName)
      })
  }

  const runLocal = function () {
    return new suiteLocal.Runner(configResolver).run()
  }
