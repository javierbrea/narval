...
const readYaml = function (fileContent) {
  return Promise.resolve(yaml.safeLoad(fileContent, { schema: yamlinc.YAML_INCLUDE_SCHEMA }))
}
...
const addCustomDockerEnvVars = function (vars, confObj = {}) {
  if (confObj.docker) {
...
const calculateAllDockerCustomEnvVars = function (suites) {
  ...
  _.each(suites, (suiteType) => {
    _.each(suiteType.suites, (suite) => {
      addCustomDockerEnvVars(customVars, suite.test)
      addCustomDockerEnvVars(customVars, suite.before)
      _.each(suite.services, (service) => {
        addCustomDockerEnvVars(customVars, service)
      })
    })
  })
...
const SuiteResolver = function (suiteData, suiteTypeName, options, suitesByType) {
  ...
  const getServiceData = function (serviceName) {
    ...
    throw Boom.notFound(logs.serviceNotFound({
      name: serviceName
    }, false))
  }
  ...
  const dockerEnvVars = function () {
    ...
    if (!_testDockerContainer) {
      throw Boom.notFound(logs.noDockerTestConfig(false))
    }
