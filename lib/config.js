...
const SuiteResolver = function (suiteData, suiteTypeName, options) {
  ...
  const localBeforeCommand = function () {
    return suiteData.before && suiteData.before.local && suiteData.before.local.command
  }

  const localTestWaitOn = function () {
    return suiteData.test.local && suiteData.test.local['wait-on']
  }

  const localTestIsCoveraged = function () {
    return !suiteData.coverage || ((!suiteData.coverage.from || suiteData.coverage.from === 'test') && suiteData.coverage.enabled !== false)
  }

  const localBeforeEnvVars = function () {
    const envVars = Object.assign({}, baseEnvVars, {
      narval_service: 'before',
      narval_is_docker: false
    })
    const customEnvVars = suiteData.before && suiteData.before.local && suiteData.before.local.env
    return Object.assign(envVars, customEnvVars)
  }

  const singleServiceToRun = function () {
    // TODO, return serviceConfigResolver, not service name
    return _.isString(options.local) && options.local !== 'test' ? options.local : false
  }

  const services = function () {
    // TODO, return array of services configs resolvers, not services name
  ...
  return {
    ...
    localTestWaitOn: localTestWaitOn,
    localTestIsCoveraged: localTestIsCoveraged,
    localBeforeCommand: localBeforeCommand,
    localBeforeEnvVars: localBeforeEnvVars,
    services: services,
    // TODO, remove next methods, published for gradual refactor
    suite: () => {
      return suiteData
    },
    options: () => {
